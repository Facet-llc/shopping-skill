...
### Changed
- `facet_mpp_charge` no longer mints a Stripe test token for a `stripe/charge` MPP challenge in the published build. It refuses a card challenge and points to the real paths: `facet_buy` for USDC, or the Stripe Link flow (`link-cli mpp pay`) for the card. The test-only card affordance stays in the development repository.

### Added
- Public GA build is mainnet-only: the client's non-production network profile (its Base sandbox plane) is removed, so the published skill resolves and settles only on Base mainnet, and the receipt renderer no longer carries a non-production network chip. The sandbox plane stays in the development repository for testing. A CI gate now blocks any non-production network reference from entering the published skill. How a real mainnet purchase settles is unchanged.
...
- `integrations/commerce-agents/facet-storefront`: a Python `StorefrontBackend` adapter that puts Anthropic's `commerce-agents` Shopping Agent on top of a Facet merchant Terminal. It implements the eleven `StorefrontBackend` methods against the Terminal's agent-facing surface (`/v1/search`, `/v1/get_product`, `/v1/order_history`) with the customer's Facet KYA on every read, holds the cart per session, and fills `checkout_handoff` with a real Facet UCP checkout-session URL so the customer completes payment non-custodially and the agent never sees the URL or moves money. First-party merchants are routed through the platform origination surface for the co-signature, with a buyer-direct fallback. Ships with offline mapping and cart tests plus a live smoke test; proven end to end against Pecan & Petal (search to a real `/ucp/v1/checkout-sessions/{id}` handoff). This is the distribution play: any `commerce-agents` agent can settle through Facet by pointing at this backend.
- `facet_storefront.server`: the commerce-agents storefront MCP server wired to run over `FacetStorefrontBackend`, so the shopping front half (catalog search, product details, cart, orders, policies, fulfillment, and customer memory, all `<storefront_data>` fenced and provenance gated) becomes MCP tools any agent connects to, while identity, checkout, and settlement stay Facet. This is the "commerce-agents as the front half, Facet as the back half" integration: `python -m facet_storefront.server` exposes the front-half tools on `127.0.0.1:8200/mcp`, customer memory is persisted by commerce-agents' `JsonFileMemoryStore`, and checkout hands off to a Facet UCP checkout session. Verified: the server builds and registers all thirteen storefront tools over the Facet backend.
- `muse/`: native [Meta Muse Code](https://developer.meta.com/ai/products/muse-code/) support. Muse (1.3.0+) has no separate plugin format, so a plugin there is a registered skill plus a registered MCP server, which is exactly what this repo already is. Adds `muse/install.ts`, a one-command Deno installer that runs `muse skills install <repo> --scope user` and adds a `facet-shopping` entry under `mcpServers` in `~/.config/muse/settings.json` (which requires a top-level `schema_version`), preserving any existing settings; a secret-free launcher `muse/facet-shopping.sh` that runs the same stdio MCP server with the wallet key read from the environment (never stored); `muse/settings.example.json` for manual setup; and `muse/README.md` covering one-command and manual install, updating, uninstalling, and troubleshooting. The `facet_*` tools run in Muse over the same server, and `SKILL.md` validates unchanged (`muse skills validate` reports `valid  shopping`). The top-level README gains a Meta Muse Code subsection under Quick start.

## [1.4.1] - 2026-09-03

### Added
- An in-chat agent-ready badge cue was trialed in this version and pulled: the roughly 700-character data-URI SVG could not be reliably emitted by the skill's model (it truncated into a broken image in real walks), so the skill no longer shows an inline mark. The hosted `facet.llc/badge/agent-ready.svg` and companion `green.html` badge assets remain for browser and website use, where an external image loads normally.
...
### Changed
- The rendered receipt (`facet_render_receipt`) now labels the settlement block by the rail that produced it, from the one template rather than a per-rail fork. A card (`card/stripe`) order reads "Rail: Card (Stripe)", "Stripe charge: `ch_...`", and "Livemode: real card charge"; a Boson order still reads "Escrow / Boson exchange", and x402 "Settlement / x402 direct". Previously a card order fell through the crypto-rail defaults and mislabelled its Stripe charge as "Escrow: settlement `ch_...`" under a raw `card/stripe` rail chip. One template, rail-conditional labels, so a layout or verify-seal change is made once and cannot drift across rails.
- SKILL.md now requires a Markdown TABLE (Item / Price / Status, with a check mark for in stock and a clear sold-out marker) whenever the agent shows more than one product or a multi-item cart, instead of the vague "a concise list" / "a short markdown block" guidance that produced a formatted table sometimes and a plain-text dump other times. A single item can still be one line. This makes the catalog and cart output consistent at a glance.
- The Stripe Link card onboarding in SKILL.md is now a smooth, ordered sequence instead of a single "run `link-cli onboard`" line: check `link-cli auth status`, start `link-cli auth login` and relay the returned `verification_url` and short phrase to the user, poll until `authenticated: true`, then confirm a card with `link-cli payment-methods list`. "One-time setup" now documents installing `@stripe/link-cli` up front and optionally registering it as an MCP server (`link-cli mcp add`) so its tools (`auth_status`, `auth_login`, `mpp_pay`, ...) are callable directly alongside the Facet tools, rather than the agent improvising the install and auth loop mid-purchase.
...
### Fixed
- Card purchases no longer gate the reservation on the wallet's USDC balance. `facet_buy` now takes `rail: "card"` (CLI `--rail card`): the DRY run creates the checkout session and returns its `checkout_id` plus the server-derived `total`, then STOPS, without selecting an on-chain rail, pre-signing a USDC transfer, or checking the wallet's USDC balance. Previously the only way to hold a card reservation was a plain `facet_buy` DRY, which always picked an on-chain rail and died on `insufficient USDC` before returning the `checkout_id`, so a card buyer whose wallet could not cover the price in USDC could not even get a quote, even though the card settles through Stripe Link (link-cli) and spends no USDC. On the card rail the wallet is only the buyer's wallet-bound KYA identity, never the funding source. link-cli cannot mint the reservation itself (it carries no Facet KYA), so the hold runs through `facet_buy`; the USDC path (no `rail`, or the on-chain rails) is unchanged and still enforces the balance check. Proven live on LaRuce: a $330 card reservation held from a wallet holding $105 USDC.
