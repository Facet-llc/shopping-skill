...
- Inline agent-ready badge cue: a small green terminal-prompt mark (animated, cursor blinking) served from `facet.llc/badge/agent-ready.svg`, shown to the left of three messages, when a store is confirmed agent-ready, at the held reservation quote, and at order completion with the receipt. Sized for inline emoji scale so a plain markdown image renders it correctly; a companion `green.html` variant is served for storefront-header iframe embeds.
