...
- Agent-ready badge cue: a small green terminal-prompt mark (animated, cursor blinking), embedded as a self-contained data-URI SVG so it renders and blinks in the chat surface. A hosted image URL is not used here because the chat surface shows it as a "Show Image" placeholder; the embedded data-URI has no external fetch and renders directly. A markdown image takes its own line in the chat surface, so the mark renders as a small badge directly above each of three messages, when a store is confirmed agent-ready, at the held reservation quote, and at order completion with the receipt. Sized for a small emoji scale. To render reliably, the skill sources the mark from an `agent_ready_badge` field the `facet_discover` and `facet_buy` (DRY and SETTLE) tool results now carry, and copies that value verbatim, rather than reproducing the roughly 700-character data-URI from the skill doc (which truncated into a broken image mid-walk). The hosted `facet.llc/badge/agent-ready.svg` and a companion `green.html` variant remain for storefront-header and website embeds.
