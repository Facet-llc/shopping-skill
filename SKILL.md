...
Card payments (optional): only needed to offer the card rail at Stripe-bound merchants (where `facet_discover` shows `mpp_method: stripe/charge`); the USDC rail needs nothing beyond the above. Install Stripe's Link CLI once with `npm install -g @stripe/link-cli` (Node, separate from this Deno skill). Optionally register it as an MCP server with `link-cli mcp add` so its tools are callable directly alongside the Facet tools, and `link-cli skills add` installs Link's own per-command guidance. The full flow is in "Paying by card (Stripe Link)".
...
This skill runs in six phases, the decomposition a good shopping agent follows. Each maps onto the concrete steps and tools below:

- **Search**: find the store and its products. Browse the live storefront (see "Browsing the storefront"), and when you need the catalog directly, `facet_directory` to find a Terminal, then `facet_search` or `facet_browse_storefront`. Steps 1 to 3 and 6.
- **Compare**: help the user weigh options. Read each product off the page and present them as a table with check marks, refining by what the user says ("something brighter", "under $30").
...
3. Discover and alert, and read the rails. Confirm agent-readiness by resolving the store's `agents.txt` (call `facet_discover` with `site`). If it carries a `Terminal`, tell the user plainly: this site is agent-ready and supports agent checkout. If it does not, the store is not Facet-enabled; stay in ordinary browsing and let the user check out however they normally would. Never touch the Terminal for a non-agent store. In the SAME `facet_discover` result, read which payment rails the store settles, because you present that choice in step 4 before anything is minted or quoted: a USDC-through-Facet option when `commerce_rails` includes `coin/usdc-base` (with the `checkout` endpoint present), and a card option when `mpp_method` is `stripe/charge` (or any `card/*` appears in `commerce_rails`). Record which the store serves. Many Stripe-bound stores serve both, so this is a real choice, not a default (LaRuce, for example, returns `mpp_method: stripe/charge` and `coin/usdc-base` together).
4. Offer agent checkout, present the payment methods, then choose the wallet. First ask whether they want agent checkout for this purchase; if they decline, stop here and nothing else runs. Then, when step 3 found more than one payment method, present the choice and let the user pick the rail BEFORE anything is minted or quoted (see "Paying by card (Stripe Link)"): USDC pays from the user's own wallet (`facet_buy`, settling on the merchant's rail), and card is approved in the user's Stripe Link app (`link-cli mpp pay`). Present only the methods the store actually serves, and never default to USDC when the store also serves a card rail without asking. This is enforced mechanically: at a both-rails store a DRY `facet_buy` with no `payment_method` returns `reason: "payment_choice_required"` and does NOT quote, so the choice cannot be skipped; once the user picks, pass `payment_method: "usdc"` (pay on-chain from the wallet) or `payment_method: "card"` (hold the reservation for the Stripe Link flow). The wallet is chosen either way, because identity is a wallet-bound KYA (step 5): call `facet_wallet_list`, show each label, address, and USDC balance, and carry the chosen `wallet` (the label) through the rest; for USDC that wallet is also the funds source, for card it only carries identity. On card, confirm the user has a linked card here, and if not, send them to `link-cli onboard` (a one-time setup) before continuing. This is the handoff and the "pick your card" step, and it happens now, at discovery, not before browsing.
...
7. Quote and price it (DRY). The rail must already be chosen (step 4) before this first DRY quote; when the store advertises both USDC and card, never reach this step having defaulted to USDC without asking. For a card purchase, this DRY `facet_buy` only HOLDS the priced reservation (use its `checkout_id`), and payment then runs through `link-cli mpp pay`, not a `facet_buy` settle (see "Paying by card (Stripe Link)"). For USDC, call `facet_buy` with `terminal`, `items`, `ship`, and the chosen `wallet`, and omit `settle`. This creates the UCP checkout, reads the merchant's offer, verifies it against the guardrails, signs the payment authorization locally, and stops. It returns the exact `price_usdc` and a `confirm_atomic` value. Nothing has moved. Show the user a thin confirmation (item, price, terms), not a rebuilt storefront (see "The confirmation surface"). If this is a gift, ask the user for a card message and a delivery date first and pass them as `gift_message` and `delivery_date` (`YYYY-MM-DD`), and `occasion` if useful: the signed UCP checkout supports these order attributes and carries them to the merchant order, so the message lands on the order and a future delivery date is requested. They do not change the price. The DRY result also carries `shipping_email_pref`; if it is `unset`, ask the buyer once about shipping confirmations before you settle (see "Shipping confirmations"), record the answer with `facet_email_pref`, then re-run the DRY `facet_buy` so the stored choice is applied.
...
- Show the user visually: a screenshot or a rendered view, plus the products as a Markdown TABLE, never a plain-text list. Use the columns Item, Price, Status, one row per product: put a check mark (✅) in Status for in stock and a clear marker for sold out (for example ⛔ sold out), and show the sale price next to the regular when on sale. Present a table EVERY time you show more than one product, not only sometimes: a table with check marks reads at a glance where a run of text does not. A single product can be one line. Let the user see the options and pick.
...
Format it as a Markdown table for more than one line: the same Item / Price / Status columns as the catalog, one row per item, then a total row, so the cart reads at a glance. A single item can be one line. A table, not a plain-text dump. Do not build image grids, do not fetch and inline photos, do not assemble a multi-item gallery. If the user wants to see the product again, send them back to the merchant's real page in the browser. The render panel is a confirmation surface, not a shopping mall.
...
When the store serves both a USDC rail and a card rail, `facet_buy` FORCES the choice: a DRY call with no `payment_method` returns `reason: "payment_choice_required"` (both options listed) instead of a quote, so you cannot silently default to USDC. Present USDC versus card to the user, then pass `payment_method: "usdc"` to quote and settle on-chain, or `payment_method: "card"` to hold the reservation for the Stripe Link flow (see "Paying by card (Stripe Link)"). A single-rail store quotes directly with no choice needed, and a forced `FACET_RAIL` or `rail: "card"` also satisfies the gate.

DRY (default, safe): call `facet_buy` with (add `"payment_method": "usdc"` at a both-rails store):
...
## Paying by card (Stripe Link)

At a merchant whose MPP method is `stripe/charge` (a Stripe-bound store, where `facet_discover` shows `mpp_method: stripe/charge`), the user has a SECOND way to pay: a card, through Stripe Link. This is the card option alongside the USDC option (`facet_buy` on the merchant's own rail). Present both and let the user choose BEFORE you mint anything: USDC means the user's own wallet signs, card means the user approves a spend in their Link app. Present only the one the merchant actually serves: a USDC-only or Boson-escrow merchant has no card option, and a merchant that does not advertise the checkout has no USDC-through-Facet option.

The card path is a composition, not new Facet code. Facet supplies the reservation and the merchant's MPP endpoint; Stripe's Link CLI (`@stripe/link-cli`, whose per-command skills install as `link-cli-*`, see "One-time setup") supplies the card, the user's approval, and the payment. The non-custodial line holds end to end: the card lives in Link, the user approves in the Link app (the agent cannot self-approve), and the only thing that reaches Facet is a one-time, challenge-bound Shared Payment Token (SPT) that settles as a direct charge on the MERCHANT'S own connected Stripe account (Facet takes only an application fee). No card number, no Link login, and no reusable token ever passes through this skill or a Facet server.

You set up the card connection once and only guide it: you never see or type the card or the Link login, that is Link's surface. Once `@stripe/link-cli` is installed (see "One-time setup"), before the first card purchase:

1. Check the connection: `link-cli auth status`. If it shows `authenticated: true`, a card is already linked, so skip to paying.
2. If not, start the device authorization: `link-cli auth login`, then read `link-cli auth status` for its `verification_url` and short `phrase`, and relay BOTH to the user. They open the URL, confirm the device reads "Claude (Facet Shopping)" and the phrase matches, and approve in their Link app.
3. Poll `link-cli auth status` until it shows `authenticated: true` (the pairing window is about 10 minutes), then confirm a card is present with `link-cli payment-methods list`.

Two conveniences: `link-cli onboard` is Link's guided one-shot (authenticate and verify a card in one command), and `link-cli mcp add` registers link-cli as an MCP server so these become tools you call directly (`auth_status`, `auth_login`, `payment-methods_list`, `mpp_pay`, `report`) instead of shelling out. If a later purchase reports the user is not authenticated, run `link-cli auth status`, then `link-cli auth login` if needed.

Per purchase, three steps:

1. Hold the reservation. Call `facet_buy` in DRY mode with `rail: "card"` (omit `settle`) for the cart and shipping address. Its result carries `checkout_id` (the reservation) and the `total`. `rail: "card"` holds the priced reservation and returns the total WITHOUT gating on the wallet's USDC balance: on the card rail the wallet is only the buyer's IDENTITY (a wallet-bound KYA), never the funding source, so a card buyer with little or no USDC can still reserve. (link-cli cannot mint the reservation itself, it carries no Facet KYA, which is why the hold runs through `facet_buy`.) Show the user the `total` and confirm before paying.
2. Pay by card through Link. Run the Link CLI's `mpp pay` against the merchant's Facet MPP endpoint, passing the reservation id as the POST body. `mpp pay` runs the whole 402 flow itself (probe, parse the challenge, create the spend request, get the user's in-app approval, pay with the SPT):

   ```
   link-cli mpp pay https://<merchant>.facet.llc/mpp/v1/charges \
     --method POST \
     --data '{"reservation_id":"<checkout_id>"}' \
     --context "<what the user is buying and why, at least 100 characters, since the user reads this text when approving in the Link app>"
   ```

   The amount and the merchant's agentic profile are server-derived from the reservation, never anything you write. On approval, Facet's Terminal settles the card charge on the merchant's connected Stripe account and returns the receipt. If the challenge is not `stripe/charge`, `mpp pay` fails closed rather than pay the wrong way.
3. Report and receipt. Run `link-cli report` with the outcome (call it after every purchase attempt), then fetch and verify the Facet receipt for the order the same as any order.

You do NOT use `facet_mpp_charge` for a real card. `facet_mpp_charge` mints a TEST Shared Payment Token (only when `FACET_STRIPE_SANDBOX_SK`, a Stripe `sk_test_` key, is set) so the card rail is testable with no Link account; it is not the production card path. A real user's card is always the Link flow above.
