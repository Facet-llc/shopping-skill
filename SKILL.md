...
You do NOT use `facet_mpp_charge` for a real card. `facet_mpp_charge` does not charge a card at all: a `stripe/charge` challenge refuses and points to the Link flow. A real user's card is always the Link flow above.
