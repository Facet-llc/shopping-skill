...
### The agent-ready mark (small inline cue)

Facet serves a tiny animated badge (a green terminal prompt with a blinking cursor) at `https://facet.llc/badge/agent-ready.svg`, sized for inline emoji scale. Show it as a markdown image immediately to the LEFT of the message text (same line, not its own line) at exactly three moments, and nowhere else:
...
Form: `![Agent ready on Facet](https://facet.llc/badge/agent-ready.svg) <your message>`. Do not resize it, do not put it on its own line, and never show it on a non-Facet store or per product row. The cursor blinks where the surface animates SVG images and shows a static green mark otherwise; both are fine.
