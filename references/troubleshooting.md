...
subcommand prints one JSON object to stdout; the fields below appear there.
...
**`discover` returns `facet_enabled: false` or `agents.txt 404`.** The site is not
Facet-enabled. Shop it normally in the browser; do not call the Terminal.
...
`--near <lat,lng>`, `--capabilities`, `--taxonomy`, `--min-reputation`, or
`--claimed-only`.
...
cap. Raise `--max-usdc` only if the cart genuinely needs it and the user has
confirmed. The cap cannot exceed the absolute ceiling.
...
match. Re-run the dry `buy`, show the user the new total, and settle with the exact
confirm value it prints.
...
moved. Do not retry `--settle`. Verify the order and on-chain state first, then act
on what you find.
...
On CHECKOUT (`buy --settle`), the store will not settle a buyer-only checkout. For a
first-party (`.facet.llc`) merchant this is handled by routing through platform
origination; for a non-first-party store, a buyer-only client cannot supply the
co-signature, and the client reports this rather than routing around it. On a REFUND
request this no longer applies: the client authorizes the request with a single-use,
order-bound wallet attestation it signs locally, so `refund` opens the ticket on a
dual-auth store with no platform in the loop (see the `refund` entry in SKILL.md). If
you still see this error on a refund, that store's Terminal has not enabled the
autonomous buyer path, and the refund needs the merchant or platform to act.
